...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const qrCodeId of qrCodeIds) {
      ...
      // Generate new QR code image with center overlay
      const qrCodeBase64 = await generateQRCodeWithOverlay(existingQR.code);
...
export async function PUT() {
  try {
    ...
    for (const qrCode of allQRCodes) {
      // Generate new QR code image with center overlay
      const qrCodeBase64 = await generateQRCodeWithOverlay(qrCode.code);
